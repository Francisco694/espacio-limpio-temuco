...
import {
    initializeApp, getApp, deleteApp
...
// 2. CONSTANTES
// ==============================================================
// Centraliza los "números mágicos" usados en distintos módulos para
// facilitar su mantenimiento y evitar duplicación de literales.
const CONST = {
    IVA_RATE: 0.19,
    TOAST_DURATION_MS: 3000,
    TOAST_FADE_MS: 300,
    NUMBER_ANIM_STEPS: 30,
    NUMBER_ANIM_DURATION_MS: 800,
    MOBILE_BREAKPOINT_PX: 900,
    CHART_MONTHS_RANGE: 6,
    RECENT_ITEMS_LIMIT: 5,
    NOTIF_LIST_LIMIT: 10,
    SEARCH_MIN_CHARS: 2,
    SEARCH_RESULTS_LIMIT: 3,
    DEFAULT_HORA_AGENDA: '09:00:00',
    LOGIN_PAGE: 'login.html',
    REFRESH_SIMULATED_DELAY_MS: 800
};
...
// ==============================================================
// 4. UTILIDADES CORE (Formatos y Helpers)
// ==============================================================
const Utils = {
    ...
    animateNumber: (selector, target) => {
        ...
        const steps = CONST.NUMBER_ANIM_STEPS;
        let current = start;
        let step = 0;
        ...
        }, CONST.NUMBER_ANIM_DURATION_MS / steps);
    },
...
// ==============================================================
// 5. UI CONTROLLER (Manejo de la Interfaz Visual)
// ==============================================================
window.UI = {
    ...
    showToast(message, type = 'success') {
        ...
        toast.innerHTML = `<i data-lucide="${icon}" class="${color} w-5 h-5 flex-shrink-0"></i><span>${Utils.escapeHtml(message)}</span>`;
    ...
    toggleSkeleton(active) {
        document.querySelectorAll('[data-kpi], table tbody tr td, .chart-wrap').forEach(el => {
            el.classList.toggle('skeleton', active);
        });
    },
...
// ==============================================================
// 6. CALCULADORA DINÁMICA (Cotizaciones)
// ==============================================================
const Calculator = {
    init() {
    ...
    calculate() {
        ...
        if (modo === 'auto') {
            subtotal = this._calcularModoAutomatico(inputSubtotal);
        } else {
            subtotal = this._calcularModoManual(inputSubtotal);
        }

        const descuento = Number(document.getElementById('cot-descuento')?.value) || 0;
        let netoFinal = subtotal - descuento;
        if (netoFinal < 0) netoFinal = 0;

        const iva = Math.round(netoFinal * CONST.IVA_RATE);
        const total = netoFinal + iva;

        this.updateUI(iva, total);
    },

    // Lógica por Cálculo Automático (tarifas dinámicas desde Firestore)
    _calcularModoAutomatico(inputSubtotal) {
        if (inputSubtotal) {
        ...
        const m2 = Number(document.getElementById('cot-m2')?.value) || 0;
        const personal = Number(document.getElementById('cot-personal')?.value) || 1;

        const base = baseValues[servicio] || 0;
        let subtotal;

        if (servicio === 'electro') {
            subtotal = base + ((personal - 1) * cfg.extra_personal);
        } else {
            const factorM2 = servicio === 'post' ? cfg.m2_post : (servicio === 'oficina' ? cfg.m2_oficina : cfg.m2_hogar);
            const costoPorM2 = m2 * factorM2;
            // Si el cálculo por m2 da menos que la base mínima (o si hay 0 m2), se cobra la base
            const costoServicio = costoPorM2 > base ? costoPorM2 : base;
    ...
    _calcularModoManual(inputSubtotal) {
        if (!inputSubtotal) return 0;
        ...
        return Number(inputSubtotal.value) || 0;
    },
    ...
    updateUI(iva, total) {
        ...
        if (cotValorTotal) cotValorTotal.value = total;
    }
};
...
const Notifications = {
    ...
    render() {
        ...
        list.innerHTML = AppState.notificaciones.slice(0, CONST.NOTIF_LIST_LIMIT).map(n => `
            <div class="notif-item ${!n.read ? 'unread' : ''}">
                ...
                <div>
                    <p class="text-sm font-bold text-slate-800">${Utils.escapeHtml(n.title)}</p>
                    <p class="text-xs text-slate-500 mt-1">${Utils.escapeHtml(n.message)}</p>
    ...
    markAllRead() {
        ...
        this.render();
        UI.showToast('Leídas', 'info');
    }
};

// ==============================================================
// 8. SECURITY MANAGER (Sesión, RBAC y Restricciones Visuales)
// ==============================================================
const SecurityManager = {
    // Valida la sesión, carga el rol del usuario desde Firestore y aplica
    // las restricciones visuales correspondientes. Devuelve `true` si el
    // acceso fue concedido y la app puede continuar inicializándose.
        ...
        try {
            ...
            if (!snap.empty) {
                ...
                if (userData.estado === 'bloqueado') {
                    ...
                    await signOut(auth);
                    window.location.replace(CONST.LOGIN_PAGE);
                    return false;
                }
            } else {
            ...
            this._aplicarRestriccionesVisuales();
            return true;
        } catch (error) {
            console.error('Error validando rol (SecurityManager):', error);
            return false;
        }
    },

    // Inyecta nombre/rol en la UI y oculta las secciones restringidas para colaboradores
    _aplicarRestriccionesVisuales() {
        const nameEl = document.getElementById('admin-name');
        const roleEl = document.getElementById('admin-role');
        if (nameEl) nameEl.textContent = AppState.user.name;
        if (roleEl) roleEl.textContent = AppState.user.role === 'admin' ? 'Administrador' : 'Colaborador';

        if (AppState.user.role !== 'admin') {
            ['nav-reportes', 'nav-ajustes', 'nav-colaboradores'].forEach(id => document.getElementById(id)?.classList.add('hidden'));
...
// ==============================================================
// 9. FIREBASE DATABASE CONTROLLER (Firestore + Auth)
// ==============================================================
window.DB = {
    // Suscribe la app a todas las colecciones necesarias en tiempo real
    initDataListeners() {
        ...
        onSnapshot(query(collection(db, 'usuarios'), orderBy('creadoEn', 'desc')), snap => {
            AppState.usuarios = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        ...
        onSnapshot(qCotiz, (snapshot) => {
            ...
            AppState.cotizaciones = snapshot.docs.map(d => ({
                id: d.id, ...d.data(), creadoEnDate: d.data().creadoEn?.toDate?.() || new Date()
            }));
        ...
        onSnapshot(query(collection(db, 'clientes'), orderBy('creadoEn', 'desc')), snap => {
            AppState.clientes = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        ...
        onSnapshot(collection(db, 'citas'), snap => {
            AppState.citas = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            Views.renderDashboard();
        });
    },

    // Crea la cuenta de Firebase Authentication del colaborador y su perfil
    // de permisos (RBAC) en Firestore. Usa una instancia secundaria de la
    // app de Firebase para no reemplazar la sesión del administrador que
    // está creando la cuenta (createUserWithEmailAndPassword inicia sesión
    // automáticamente con el usuario recién creado en la instancia usada).
    async saveColaborador(e) {
        e.preventDefault();

        const nombreInput = document.getElementById('colab-nombre');
        const correoInput = document.getElementById('colab-correo');
        const rolInput = document.getElementById('colab-rol');
        const passInput = document.getElementById('colab-pass');
        const btn = document.getElementById('btn-save-colaborador');

        const nombre = nombreInput?.value.trim() || '';
        const correo = correoInput?.value.trim().toLowerCase() || '';
        const rol = rolInput?.value || 'colaborador';
        ...
        if (btn) { btn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Creando...'; btn.disabled = true; }

        // Nombre único para evitar colisiones si se crean varios colaboradores seguidos
        const secondaryAppName = `ColaboradorSecondary_${Date.now()}`;
        let secondaryApp = null;

        try {
            // 1) Crear la cuenta real en Firebase Authentication (app secundaria)
            secondaryApp = initializeApp(getApp().options, secondaryAppName);
            const secondaryAuth = getAuth(secondaryApp);
            const credential = await createUserWithEmailAndPassword(secondaryAuth, correo, pass);

            // 2) Guardar el perfil de permisos (RBAC) en Firestore usando el mismo UID
            ...
            UI.showToast('Colaborador registrado exitosamente (cuenta y permisos creados)');
        } catch (error) {
            ...
            if (secondaryApp) {
                try { await deleteApp(secondaryApp); } catch (cleanupErr) { console.warn('No se pudo limpiar la app secundaria:', cleanupErr); }
            }
    ...
    async saveConfiguracion() {
        ...
        try {
            ...
            const dataConfig = {
                base_hogar: Number(document.getElementById('cfg-base-hogar')?.value) || 0,
                base_oficina: Number(document.getElementById('cfg-base-oficina')?.value) || 0,
                base_post: Number(document.getElementById('cfg-base-post')?.value) || 0,
                base_electro: Number(document.getElementById('cfg-base-electro')?.value) || 0,
                m2_hogar: Number(document.getElementById('cfg-m2-hogar')?.value) || 0,
                m2_oficina: Number(document.getElementById('cfg-m2-oficina')?.value) || 0,
                m2_post: Number(document.getElementById('cfg-m2-post')?.value) || 0,
                extra_personal: Number(document.getElementById('cfg-extra-personal')?.value) || 0,
    ...
    async saveCotizacionDraft(e) {
        ...
        const total = Number(document.getElementById('cot-valor-total')?.value) || 0;

        const inputSub = document.getElementById('cot-subtotal-input');
        const subtotal = inputSub ? (Number(inputSub.value) || 0) : Math.round(total / (1 + CONST.IVA_RATE));
        ...
        const data = {
            ...
            metros: document.getElementById('cot-m2')?.value || 0,
            personal: document.getElementById('cot-personal')?.value || 1,
            descuento: document.getElementById('cot-descuento')?.value || 0,
            prioridad: document.getElementById('cot-prioridad')?.value || 'Normal',
        ...
        try {
            ...
            UI.showToast('Error al procesar', 'error');
        }
    },
    ...
    async approveCotizacion() {
        ...
        try {
            await updateDoc(doc(db, 'cotizaciones', id), {
                estadoCRM: 'Aprobada',
                modificadoEn: serverTimestamp(),
                valorTotal: Number(document.getElementById('cot-valor-total')?.value) || 0
            });
            UI.closeModal('modal-cotizacion');
            ...
            UI.showToast('Error al aprobar', 'error');
        }
    },
...
// ==============================================================
// 10. CONTROLADORES GLOBALES (Acciones Principales y RBAC)
// ==============================================================
window.Controllers = {
    ...
    editCotizacion(id) {
        ...
        this._renderEvidencias(c);

        document.getElementById('cot-modal-title').textContent = `Cotización: ${c.cliente || c.nombre}`;
        const esWeb = c.fuente === 'Modal Web' || !c.fuente;
        document.getElementById('cot-modal-source').innerHTML = esWeb
            ? '<i data-lucide="globe" class="inline w-3 h-3 mr-1"></i> Origen: Solicitud Web Pública'
            : '<i data-lucide="laptop" class="inline w-3 h-3 mr-1"></i> Origen: Manual';
        ...
        } else if (c.estadoCRM === 'Aprobada' || c.estadoCRM === 'Rechazada') {
            btnApprove.classList.add('hidden');
            btnReject.classList.add('hidden');
    ...
    // Renderiza las fotos/evidencias adjuntas a la cotización (si existen) dentro del modal
    _renderEvidencias(c) {
        const contenedor = document.getElementById('cot-evidencias');
        const seccion = document.getElementById('evidencias-section');
        ...
        if (c.evidencias?.length > 0) {
            ...
            contenedor.innerHTML = c.evidencias.map(url => `
                <div class="group relative">
                    <img
                        src="${Utils.escapeHtml(url)}"
                        class="w-full h-40 object-cover rounded-lg border shadow cursor-pointer hover:scale-105 transition"
                        onclick="window.open('${Utils.escapeHtml(url)}','_blank')">
                </div>
            `).join('');
        } else {
    ...
    exportarPDF() {
        ...
        printWindow.document.write(`
            <html>
            <head>
                <title>Cotización - ${Utils.escapeHtml(nombre)}</title>
                <style>
            ...
            <body>
                ...
                <div class="info-box">
                    <p><strong>Cliente / Empresa:</strong> ${Utils.escapeHtml(nombre)}</p>
                    <p><strong>RUT:</strong> ${Utils.escapeHtml(rut)}</p>
                ...
                <table>
                    ...
                    <tr>
                        <td>${Utils.escapeHtml(servicio)} <br><span style="font-size:11px; color:#64748b;">(Incluye personal, insumos e impuestos correspondientes)</span></td>
...
// ==============================================================
// 11. CONTROLADORES DE VISTA (Renders de Pantallas)
// ==============================================================
const Views = {
    ...
    navigate(viewId) {
        ...
        const titles = {
            panel: 'Panel Principal', clientes: 'Directorio de Clientes', agendamiento: 'Calendario Operativo',
            cotizaciones: 'Gestión de Cotizaciones', reportes: 'Reportes Avanzados', ajustes: 'Configuración del CRM',
            colaboradores: 'Gestión de Colaboradores'
        };
        document.getElementById('header-title').textContent = titles[viewId] || 'Panel';

        if (window.innerWidth < CONST.MOBILE_BREAKPOINT_PX) document.getElementById('sidebar').classList.remove('open');

        // Ejecución de renderizado condicional (incluye colaboradores)
        if (viewId === 'panel') this.renderDashboard();
        if (viewId === 'agendamiento') this.renderCalendar();
        if (viewId === 'reportes') this.renderReportes();
        if (viewId === 'colaboradores') this.renderColaboradores();
    },
    ...
    renderDashboard() {
        ...
        const ingresos = AppState.cotizaciones
            .filter(c => c.estadoCRM === 'Aprobada')
            .reduce((acc, c) => acc + (Number(c.valorTotal) || Number(c.valorCotizado?.replace(/\D/g, '')) || 0), 0);
        ...
        if (tbody) {
            const r = AppState.cotizaciones.slice(0, CONST.RECENT_ITEMS_LIMIT);
            tbody.innerHTML = r.length ? r.map(c => `
                <tr class="hover:bg-slate-50 clickable-row" onclick="Controllers.editCotizacion('${c.id}')">
                    ...
                    <td><span class="estado-badge ${UI.getBadgeClass(c.estadoCRM)}">${Utils.escapeHtml(c.estadoCRM || 'Pendiente')}</span></td>
                </tr>
    ...
    renderChart() {
        ...
        const data = Array(CONST.CHART_MONTHS_RANGE).fill(0);
        const labels = [];
        const anioActual = new Date().getFullYear();

        for (let i = CONST.CHART_MONTHS_RANGE - 1; i >= 0; i--) {
            const d = new Date();
            ...
            data[(CONST.CHART_MONTHS_RANGE - 1) - i] = ingresosMes;
        }
        ...
        AppState.chartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: { border: { display: false }, grid: { color: '#f1f5f9' }, ticks: { color: '#64748b', callback: v => `$${v / 1000}k` } },
    ...
    renderReportes() {
        ...
        AppState.cotizaciones.forEach(c => {
            const s = c.tipoLimpieza || c.servicio || '';
        ...
        const est = { aprobadas: 0, pendientes: 0, rechazadas: 0 };
        AppState.cotizaciones.forEach(c => {
    ...
    renderTimeline() {
        ...
        if (!feed) return;
        const recientes = AppState.cotizaciones.slice(0, CONST.RECENT_ITEMS_LIMIT);
    ...
    renderClientes() {
        ...
        const filtro = AppState.filtros.clientes;
        ...
        const renderStr = filtrados.map(c => `
            <tr>
                <td><div class="client-cell"><div class="client-avatar-sm">${(c.nombre || '?').charAt(0)}</div><div><p class="client-name-text">${Utils.escapeHtml(c.nombre)}</p><p class="text-xs text-slate-500">${Utils.escapeHtml(c.rut)}</p></div></div></td>
    ...
    renderCotizaciones() {
        ...
        const filtro = AppState.filtros.cotizaciones;
        ...
        const renderStr = filtrados.map(c => {
            ...
            <tr class="hover:bg-slate-50 clickable-row" onclick="Controllers.editCotizacion('${c.id}')">
                ...
                <td class="text-sm"><span class="font-semibold">${Utils.formatDate(c.fechaAgendada || c.fecha)}</span><br><span class="text-xs text-slate-500">${Utils.escapeHtml(c.horaAgendada || c.hora || '')}</span></td>
                <td class="text-sm font-semibold ${c.prioridad === 'Urgente' ? 'text-red-500' : (c.prioridad === 'Alta' ? 'text-yellow-600' : 'text-slate-500')}">${Utils.escapeHtml(c.prioridad || 'Normal')}</td>
    ...
    renderCalendar() {
        ...
        const events = AppState.cotizaciones.filter(c => c.estadoCRM === 'Aprobada').map(c => ({
            ...
            start: `${c.fechaAgendada || c.fecha}T${c.horaAgendada || c.hora || CONST.DEFAULT_HORA_AGENDA}`,
    ...
    renderColaboradores() {
        ...
        const renderStr = AppState.usuarios.map(u => {
            ...
            <tr>
                ...
                <td><span class="text-xs font-bold uppercase tracking-wider text-slate-500">${Utils.escapeHtml(u.rol)}</span></td>
                <td><span class="estado-badge border ${badgeClass}">${Utils.escapeHtml(u.estado || 'activo')}</span></td>
                <td class="text-center">
...
// ==============================================================
// 12. EVENT LISTENERS (Bindings de la Interfaz)
// ==============================================================
function setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.header-user') && !e.target.closest('.header-icon-btn') && !e.target.closest('.dropdown-menu')) {
    ...
    globalSearch?.addEventListener('input', (e) => {
        ...
        if (val.length < CONST.SEARCH_MIN_CHARS) { searchDropdown.classList.remove('active'); return; }

        const resClientes = AppState.clientes.filter(c => (c.nombre || '').toLowerCase().includes(val));
        const resCot = AppState.cotizaciones.filter(c =>
            (c.cliente || c.nombre || '').toLowerCase().includes(val) || (c.tipoLimpieza || c.servicio || '').toLowerCase().includes(val)
        );

        let html = '';
        if (resClientes.length) {
            html += `<div class="px-3 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50">Clientes</div>` +
                resClientes.slice(0, CONST.SEARCH_RESULTS_LIMIT).map(c => `<div class="search-result-item" onclick="UI.toggleDropdown('search-results-dropdown'); document.querySelector('[data-view-target=\\'clientes\\']').click();"><span class="font-bold text-sm text-slate-800">${Utils.escapeHtml(c.nombre)}</span><span class="text-xs text-slate-500">RUT: ${Utils.escapeHtml(c.rut)}</span></div>`).join('');
        }
        if (resCot.length) {
            html += `<div class="px-3 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50">Cotizaciones</div>` +
                resCot.slice(0, CONST.SEARCH_RESULTS_LIMIT).map(c => `<div class="search-result-item" onclick="UI.toggleDropdown('search-results-dropdown'); document.querySelector('[data-view-target=\\'cotizaciones\\']').click();"><span class="font-bold text-sm text-slate-800">${Utils.escapeHtml(Utils.getServicioName(c.tipoLimpieza || c.servicio))}</span><span class="text-xs text-slate-500">${Utils.escapeHtml(c.cliente || c.nombre)} • ${Utils.escapeHtml(c.estadoCRM)}</span></div>`).join('');
        }

        searchContent.innerHTML = html || `<div class="p-4 text-center text-xs text-slate-500">No se encontraron coincidencias para "${Utils.escapeHtml(val)}"</div>`;
    ...
    document.getElementById('btn-refresh')?.addEventListener('click', function () {
        ...
        }, CONST.REFRESH_SIMULATED_DELAY_MS);
    });
    ...
    const handleLogout = async (e) => {
        ...
        if (confirm('¿Estás seguro de cerrar sesión?')) {
            try {
                ...
                window.location.replace(CONST.LOGIN_PAGE); // Previene acceso por historial de navegación
            } catch (error) {
...
// ==============================================================
// 13. APP INIT (Control de Sesión y RBAC — Punto Único de Arranque)
// ==============================================================
document.addEventListener('DOMContentLoaded', () => {
    onAuthStateChanged(auth, async (user) => {
        ...
        const accessGranted = await SecurityManager.initializeUserAccess(user);
        if (!accessGranted) return;

        const dateOpts = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
        const headerDate = document.getElementById('header-date');
        if (headerDate) headerDate.textContent = new Date().toLocaleDateString('es-ES', dateOpts);
